...
// >>>> ANSWER 1 (easier way but not correct based off what the question is asking.)
// artists[8].name = 'Vincent Van Gogh';
// console.log(artists[8]);

// >>>> ANSWER 2
artists.forEach((i) => {
	if (i.name === 'Vincent van Dough') {
		return (i.name = 'Vincent Van Gogh');
	}
});
...
// 	return `The artist at index ${i} is ${found.name}.`;
// }
...
function getArtistByIndex(array, index) {
	...
	return `The artist at index ${index} is ${found[0].name}.`;
}
...
function get20s(array) {
	...
	for (let i = 0; i < newArr.length; i++) {
		if (newArr[i].years[0] >= 1900 && newArr[i].years[1] <= 2000) {
			found.push(newArr[i].name);
		}
	}
...
/*
SOLUTION 3 (student Richard Rodriguez)
